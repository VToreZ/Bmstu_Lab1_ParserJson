...
Json::Json(const string& s) {  //structure type
    unsigned int i = 0;
    i = miss_spaces(i, s);
    if (s[i] == '{') {
        _is_object = true;
...
std::any& Json::operator[](const string& key) { //return key.value (for obj)
    if (Json::is_object()) {
...
std::any& Json::operator[](int index) { // return arr[index]
    if (is_array()) {
